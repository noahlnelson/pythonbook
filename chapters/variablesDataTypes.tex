...
appear in \code{this font} in the Canopy editor window and run the code by
...
must use the \code{print} command, like this
\begin{Verbatim}
print(a)
\end{Verbatim}
...
example, what would be the value of \code{d} above if the result has
to be an integer?
...
print(" {:.45f} ".format(b))  #Formatted print: you haven't learned
                              #about this yet
\end{Verbatim}
The first print statement displays the value of \code{b} to one
...
Python to display the value of \code{b} out to 45 decimal places.
Notice that the true value of \code{b} is not \ul{exactly} equal to
...
accidentally use parenthesis (\code{()})\sidenote{Use parenthesis to
  create a tuple, which is just like a list but cannot be modified.}
or curly brackets (\code{\{\}})\sidenote{Use curly brackets to
...
\noindent Python has a built-in function called \code{range} that can be used
...
\code{a + b}  & Join list \code{a} to list \code{b} to form one list. \\ \\
...
(\code{\{\}}) are found.
 ...
 like the variable formatted when it is printed. The \code{:d} is
 used to indicate an integer variable and \code{:f} is used for
 floats.  Further specifications regarding spacing can also be made.
 The \code{5.2} in the float formatting indicates that I'd like the
...
\code{\{:8.4f\}}  & Display float with at leasat 8 total spaces and 4 numbers after the decimal\\ \\
\end{tabular}
