...
\code{plot} function (also inside of \code{matplotlib}).
\begin{Verbatim}
...
pyplot.show()
\end{Verbatim}
The \code{plot} function connects the data points being plotted.  In
...
built-in \code{range} function can't do that for us.  However, the
\code{numpy} library has a function called \code{arange} that does
just that.
\begin{Verbatim}
from math import sin
