...
\section{There Are Limits to Lists}
There are several things that seem natural to do with lists but that
should not (cannot) be done.  First, you may feel tempted to associate
a list with a mathematical vector and try to perform vector math on
them. For instance, it may seem natural to try
\begin{Verbatim}
a = [5.1 , 3.2 , 6.8 , 9.2]
b = 5 * a
\end{Verbatim}
...
up.  However, there is to be an easier way.  The easier way involves a
library called \code{numpy} .  The main object used in this library is
called an \code{array}, which is very similar to a list except that an
array is intended for mathematical use.  If \code{x} were a Numpy
array, the code would be as simple as:
\begin{Verbatim}
...
Let's return, for a moment, to the problem posed at the beginning
...
name of the array of numbers is \code{x}):
\begin{Verbatim}
D = 5
sum( (x - D)**3)
\end{Verbatim}
